...
export const editUsername = async (otp, newUsername) => {
  ...
  if (!response.ok) {
    ...
    throw new Error(errorData || 'Failed to edit the account username');
  }
...
export const editEmail = async (userID, newEmail, answers) => {
  const token = await AsyncStorage.getItem('authToken');
  ...
  if (!response.ok) {
    const errorData = await response.json();
    console.log(errorData);
    throw new Error(errorData || 'Failed to edit the account email');
  }
  ...
  export const sendDeleteVerification = async () => {
    try {
      ...
      const response = await fetch(`${BASE_URL}/owner/delete-account-request`, {
        method: 'POST', 
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `${token}`,
  ...
  export const deleteUser = async (otp) => {
    ...
    const response = await fetch(`${BASE_URL}/owner/delete-account`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        Authorization: `${token}`,
    ...
    if (!response.ok) {
      const errorData = await response.json();
      console.log(errorData);
      throw new Error(errorData || 'Failed to delete the account');
    }
  };
